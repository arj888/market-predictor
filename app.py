...
st.title("Frontier Sovereign Quant Terminal")
...
ASSET_DATABASE = {
    "Crypto": {
        ...
        "Polygon / MATIC (MATIC-USD)": "MATIC-USD",
...
selected_category = st.selectbox(
    "1. Choose Market Category:",
    list(ASSET_DATABASE.keys())
)

if selected_category != "Custom Search / Other":
    category_items = ASSET_DATABASE[selected_category]
    selected_label = st.selectbox(
        f"2. Select or Search {selected_category} Asset:",
        options=list(category_items.keys()),
        index=0
    )
    ticker = category_items[selected_label]
    asset_display_name = selected_label
else:
    ticker = st.text_input("2. Enter Any Ticker (e.g. BTC-USD, RELIANCE.NS, TSLA, GC=F, EURUSD=X):", "").strip().upper()
    asset_display_name = ticker
...
if st.button("Run Comprehensive Quantitative Prediction"):
    if not ticker:
        st.error("Please enter or select a valid asset ticker.")
    else:
        with st.spinner(f"Computing Unified Quantitative Synthesis for {asset_display_name}..."):
            asset = yf.Ticker(ticker)
            df = asset.history(period="3y", interval="1d", auto_adjust=False)

            if df.empty or len(df) < 140:
                st.error("Data fetch failed or insufficient historical trading history.")
            else:
                ...
                regime_map = {0: "Low-Vol Expansion (Bullish)", 1: "High-Vol Shock (Risk)", 2: "Mean-Reverting Equilibrium"}
                ...
                st.divider()
                st.subheader(f"Quant Synthesis Result: {asset_display_name}")
                ...
                st.markdown("### Execution Plan & Risk Guard")
                t1, t2, t3, t4 = st.columns(4)
                t1.metric("Take-Profit (2x ATR)", f"{target_1:.4f}")
                t2.metric("Stop-Loss (1x ATR)", f"{stop_loss:.4f}")
                t3.metric("95% Expected Band", f"{lower_band:.2f} - {upper_band:.2f}")
                t4.metric("Kelly Allocation", f"{safe_kelly:.1f}%")

                st.markdown("### Frontier Spectral & Math Diagnostics")
                f1, f2, f3, f4 = st.columns(4)
                f1.metric("Dominant Cycle (FFT)", f"{fft_dominant_days:.1f} Days")
                f2.metric("OU Half-Life", f"{ou_half_life_days:.1f} Days" if ou_half_life_days > 0 else "Trend")
                f3.metric("Fractal Hurst (H)", f"{hurst_val:.2f} ({fractal_state})")
                f4.metric("Walk-Forward Hit%", f"{win_rate:.1f}%")

                st.markdown("### Institutional Macro & Tail Risk Regimes")
                r1, r2, r3, r4 = st.columns(4)
                r1.metric("Market Regime", regime_label.split()[0])
